...
public class TagDropdownServlet extends SlingSafeMethodsServlet {
  ...
  protected void doGet(SlingHttpServletRequest request, SlingHttpServletResponse response) {
    ...
    for (Resource childTags : tagsResource.getChildren()) {
      ...
      Tag pokemonTypeTag = childTags.adaptTo(Tag.class);
      if (pokemonTypeTag == null) {
        log.info("pokemonTypeTag is null");
        return;
      }

      String tagFullName = pokemonTypeTag.getTagID();
      String tagName = tagFullName.substring(tagFullName.lastIndexOf("/") + 1);
      String tagTitle = pokemonTypeTag.getTitle();
